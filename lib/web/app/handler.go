...
func NewHandler(ctx context.Context, c *HandlerConfig) (*Handler, error) {
	...
	h.router.GET("/x-teleport-auth", makeRouterHandler(h.handleFragment))  // beginAuthExchange
	h.router.POST("/x-teleport-auth", makeRouterHandler(h.handleFragment)) // finishAuthExchange
