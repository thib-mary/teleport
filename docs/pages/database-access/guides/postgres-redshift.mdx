...
There are two methods of IAM authentication supported by Redshift:

<Tabs>
<TabItem label="IAM authentication as database users (recommended)">
Database Service can authenticate with Redshift by generating IAM
authentication tokens as existing database users.

The Database Service requires IAM permissions to manage its own IAM role or IAM
user policies. As a result, appropriate `redshift:GetClusterCredentials`
permissions will be added to the IAM role or IAM user of the Database Service.
</TabItem>

<TabItem label="IAM authentication as IAM roles">
Teleport can authenticate with Redshift by generating IAM authentication tokens
as IAM roles, and Redshift maps these IAM roles to database users
automatically.

The Database Service must have trust relationship with these IAM roles so the
Database Service can assume these IAM roles. Note that these IAM roles must
have `redshift:GetClusterCredentialsWithIAM` to the Redshift clusters to be
accessed.
...
<Tabs>
<TabItem label="IAM authentication as database users">
```code
