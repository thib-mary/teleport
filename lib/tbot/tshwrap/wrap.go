...
func GetEnvForTSH(destPath string) (map[string]string, error) {
	...
	mergeEnv(env, filepath.Join(destPath, identity.SSHKeyKey), client.VirtualPathEnvNames(client.VirtualPathSSHKey, nil))
	mergeEnv(env, filepath.Join(destPath, identity.TLSKeyKey), client.VirtualPathEnvNames(client.VirtualPathTLSKey, nil))
