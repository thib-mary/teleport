...
func testDraftExternalCloudAudit(t *testing.T) *externalcloudaudit.ExternalCloudAudit {
	draft, err := externalcloudaudit.NewDraftExternalCloudAudit(header.Metadata{}, externalcloudaudit.ExternalCloudAuditSpec{
...
func TestConfiguratorIsUsed(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// Just create draft, external cloud audit should be disabled, it's
			// active only when the draft is promoted to cluster external cloud
			// audit resource.
			resourceServiceFn: func(t *testing.T, s services.ExternalCloudAudits) {
				_, err := s.UpsertDraftExternalCloudAudit(ctx, draftConfig)
				require.NoError(t, err)
			},
			wantIsUsed: false,
		},
		{
			...
			resourceServiceFn: func(t *testing.T, s services.ExternalCloudAudits) {
				_, err := s.UpsertDraftExternalCloudAudit(ctx, draftConfig)
				require.NoError(t, err)
				err = s.PromoteToClusterExternalCloudAudit(ctx)
				require.NoError(t, err)
			},
			wantIsUsed: true,
		},
...
func TestCredentialsCache(t *testing.T) {
	...
	// Pre-req: existing cluster ExternalCloudAudit configuration
	draftConfig := testDraftExternalCloudAudit(t)
	svc := local.NewExternalCloudAuditService(mem)
